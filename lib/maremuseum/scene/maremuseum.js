...
import AutoRunner from "mazescript/autorunner.js";
...
export default class MusicMazeScene extends MazeObject {
	...
	*/
	...
	initMusic() {
		...
		if (!this.onMusicStartedCalled && this.onMusicStarted) {
			this.onMusicStarted();
			this.onMusicStartedCalled = true;
		}
	}
	...
	stopMusic() {
		...
		this.audio1.pause();
		this.audio2.pause();
		this.audio1.position = 0;
		this.audio2.position = 0;
	}

	/** Options:
	 *  - **onMusicStarted**:  
	 * 	Called once the user interacts and begins the game. 
	 *  - **btnMareMural**:  
	 * 	Button element to add onclick event to travel to the mare mural.
	 *  - **btnMareWander**:  
	 * 	Button element to add onclick event to wander the museum. 
	 *  - **btnMareStop**:
	 * 	Button element to add onclick event to stop autopilot.
	 */
	constructor(mazeEngine, args) {
		...
		window.addEventListener("mousedown", this.initMusic.bind(this));
		window.addEventListener("touchstart", this.initMusic.bind(this));
		window.addEventListener("keydown", this.initMusic.bind(this));
		 ...
		 * @type {AutoRunner}
		 */
		this.autoRunner = player.addScript(AutoRunner);

		if (args.btnMareMural) {
			args.btnMareMural.onclick = () => {
				if (!this.onMusicStartedCalled) {
					this.initMusic();
				}
				this.autoRunner.setMode_Destination(new THREE.Vector2(6, 18), 0);
			};
		} else {
			console.error("no btnMareMural");
		}

		if (args.btnMareWander) {
			args.btnMareWander.onclick = () => {
				if (!this.onMusicStartedCalled) {
					this.initMusic();
				}
				this.autoRunner.setMode_Wander();
			};
		} else {
			console.error("no btnMareWander");
		}

		if (args.btnMareStop) {
			args.btnMareStop.onclick = () => {
				if (!this.onMusicStartedCalled) {
					this.initMusic();
				}
				this.autoRunner.setMode_Idle();
			};
		} else {
			console.error("no btnMareStop");
		}

		if (args.btnMareStopMusic) {
			args.btnMareStopMusic.onclick = () => {
				this.stopMusic();
			};
		} else {
			console.error("no btnMareStopMusic");
		}

		if (args.btnMareStartMusic) {
			args.btnMareStartMusic.onclick = () => {
				try {
					this.stopMusic();
				} finally {
					this.initMusic();
			...
			console.error("no btnMareStartMusic");
		}

		if (args.rangeMareMusicVolume) {
			args.rangeMareMusicVolume.oninput = () => {
				this.audioMultiplier = args.rangeMareMusicVolume.value/100;
			};
		} else {
			console.error("no rangeMareMusicVolume");
		}
	...
	update() {
		...
		if (this.musicStarted) {
			...
			if (currentAudio.volume < this.audioMultiplier ) {
				try {
					currentAudio.volume += dt * this.audioMultiplier;
				} catch (_) {
					currentAudio.volume = this.audioMultiplier;
				}
			...
			if (otherAudio.volume > 0) {
				try {
					otherAudio.volume -= dt * this.audioMultiplier;
				} catch (_) {
					otherAudio.volume = 0;
				}
			} else {
